...
function prepareDeploy(dest, file, callback) {
    if(file.release) {
        ...
        if(file.isText() && content.length) {
            ...
            if(dest.opt.sync && file.isHtmlLike){
                port = port || fis.config.get('browsersync.port', fis.config.get('livereload.port', 8132));
                var code = '<script type="text/javascript" id="__bs_script__">document.write("<script async src=\'http://HOST:' + port + '/browser-sync/browser-sync-client.js\'><\\/script>".replace("HOST", location.hostname));</script>';
